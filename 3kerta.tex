...
\includegraphics[height=4cm,angle=180]{smile}<>
    ...
    \begin{minipage}{5cm}
        \centering
        \includegraphics[height=3.5cm,angle=180]{smile}
    \end{minipage}
    \pause
    \par\smallskip
...
\begin{fframe}
    \frametitle{Figure-ympäristön sijainti}
    Kelluvien ympäristöjen sijoittaminen johonkin tiettyyn paikkaan voi olla haastavaa. Yleensä kannattaa antaa \LaTeX{}in sijoittaa ympäristö haluamaansa paikkaan ja viitata siihen tekstistä \lstinline-\ref- ja \lstinline-\pageref- komennoilla.
    \pause
    \vaihto
    Kelluvan ympäristön sijoittaminen ''tähän'' (siihen, mihin sen koodin perusteella kuvittelisi tulevan) on kuitenkin mahdollista käyttämällä \cns{float}-pakettia. 
    \pause
    Kun paketti on käytössä, \cns{figure}-ympäristölle voi antaa valinnaisen argumentin \cns{H} (\textsl{Here}), jolloin se ei lähde seilaamaan:
    \begin{lstlisting}
\begin{figure}[H]
    ...
\end{figure}<>
...
\begin{fframe}
    ...
    \begin{harj}
        Tuo työhösi pari kuvaa käyttäen \cns{figure}-ympäristöä. Anna \LaTeX{}in päättää toisen sijoittamisesta ja sijoita toinen ''tähän''. Lisää vielä kuvateksti ainakin sille, jonka sijainnista ei ole tietoa.
    \end{harj}
    \begin{extra}
        Kuvan sijoitteluun voi vaikuttaa myös ilman \cns{float}-pakettia antamalla valinnaiseksi argumentiksi osan tai kaikki merkeistä \cns{!htbp}. Nämä kertovat, mihin sijoittaminen \emph{sallitaan}. Merkillä \cns{h} sijoittaminen sallitaan ''tähän'', merkillä \cns{t} (jonkin) sivun yläreunaan, merkillä \cns{b} sivun alareunaan ja merkillä \cns{p} omalle sivulleen. Merkki \cns{!} tarkoittaa, että kauneudesta voi tarvittaessa tinkiä (mutta ei kovin paljoa).
        \vaihto
        Esimerkiksi \cns{!hb} tarkoittaa, että kuvan voi sijoittaa ''tähän'' tai sivun alareunaan ja että rumuuttakin sallitaan. Argumentti \cns{h} (tai \cns{!h}) \emph{ei} toimi samalla tavalla kuin \cns{float}-paketin \cns{H} syistä, jotka jätetään uteliaitten itse selvitettäviksi.
    \end{extra}
\end{fframe}
...
\end{table}<>
    \end{lstlisting}
    \pause
    Ympäristön \cns{table} sijoittelu noudattaa samoja sääntöjä kuin \cns{figure}. Erityisesti \cns{float}-paketin kanssa sille voi antaa valinnaisena argumenttina \cns{H}:n, jolloin taulukko tulee ''tähän''.
\end{fframe}
...
\begin{fframe}
    \begin{harj}
        ...
        Tee tehtävässä \ref{taulukko} luomastasi taulukosta kelluva ja anna sille jokin otsikko.
    \end{harj}
\end{fframe}
