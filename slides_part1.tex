...
\[
\verb-amsthm-,\quad \verb-amsfonts-,\quad \verb-amsmath-\quad \text{ja}\quad \verb-amssymb-
\]
...
\end{harj}
\pause
Nämä sisältävät fontteja, symboleita ja muuta matemaattisen tekstin kirjoittamiselle tarpeellista. 
\end{frame}
\begin{frame}[fragile]
...
Muotoa 
\[
\verb-\begin{ympäristön nimi}-\ldots \verb-\end{ympäristön nimi}-.
\]
...
\begin{itemize}[<+->]
\item \% aloittaa kommenttirivin
\item \$ aloittaa ja lopettaa tavallisen matematiikkatilan
\item \(\backslash\) aloittaa komennon (komento \verb-\\- katkaisee rivin)
\item \& on käytössä kun rivejä järjestetään kohdakkain
\item \{ ja \} ovat komennon argumentin ympärille tulevat merkit
\end{itemize}\pause
...
\begin{frame}[fragile]
\begin{harj}
Kolme peräkkäistä pistettä saa komennolla \verb-\ldots-. Kokeile, miten lopputulokset eroavat, jos kirjoitat pisteet itse.  Kirjoita sitten seuraava:
\begin{framed}
Erikoismerkkejä ovat mm. \%, \$ ja \&. Merkkijonon \textbackslash textbackslash tuottaminen onnistuu näin\ldots
\end{framed}
\end{harj}
\begin{harj}
...
\item rivinvaihto ei tee mitään
...
\vaihto\pause Pystysuunnassa tyhjää tilaa saa komennolla \verb-\vspace{mitta}-, jossa mitta annetaan pikseleinä (pt) tai senttimetreinä (cm). 
\end{frame}
%\begin{frame}[fragile]
%\frametitle{Tekstin muokkaaminen}
...
\vaihto\pause
Tekstin \textbf{lihavointi} onnistuu manuaalisesti komennolla \verb-\textbf{lihavoitava teksti}-, \textit{kursivointi} komennolla \verb-\textit{kursivoitava teksti}- ja \underline{alleviivaus} komennolla \verb-\underline{alleviivattava teksti}-. 
\vaihto\pause
...
Tuota seuraava dokumenttiisi:
\begin{framed}
Jos \(F\) on \(\sigma\)-algebra ja \(A_i\in F\) kaikilla \(i= 1,2,\ldots\), niin 
\[
...
\end{framed}
\end{harj}
Komennolla \verb-\middle|- saat pystyviivan automaattisesti oikean kokoisena. Tehtävästä \ref{rajaArvo} saat apua symboleiden \(\in\) ja \(\Z\) luomiseen.
\end{frame}
\begin{frame}[fragile]
\begin{harj}
Kirjoita seuraava:
\begin{framed}
Jos \(F\) on \(\sigma\)-algebra ja \(P\colon F\to\R\) todennäköisyys, niin tapahtumille \(A_1,A_2,\ldots\in F\) pätee
\[
...
%\end{minipage}
\begin{harj}
Kirjoita muutaman yhtälön ketju ja sijoita yhtälöt allekkain, tasaten haluamastasi kohdasta. Voit esimerkiksi derivoida vaiheittain funktion \(x^3\sin(\cos(x))\) tai keksiä jotkin muut yhtälöt. Yhtälöiden ei tarvitse olla tosia.
\end{harj}
\begin{harj}
...
\end{frame}
\begin{frame}[fragile]
\begin{harj}\label{viittausTehtävä}
Tuota seuraava dokumenttiisi:
\begin{framed}
...
\begin{harj}
Viittaa harjoituksessa \ref{viittausTehtävä} kirjoittamaasi numeroituun yhtälöön. Käytä komentoja \verb-\eqref{}- ja \verb-\pageref{}-.
\vaihto
...
\end{Verbatim}
luo komennon \verb-\R-, joka tulostaa symbolin \(\R\). Komennon luomisen jälkeen kyseisen symbolin tuottaminen onnistuu helposti.
\vaihto
